...
def export_json():
    try:
        ...
        # Remove large description fields for cleaner export
        export_data = {
...
def share():
    try:
        data = request.get_json()
        if not data:
            return jsonify({'error': 'No data to share'}), 400
        
        # Create a shareable link (encode data in base64)
        share_data = {
            'query': data.get('query'),
            'type': data.get('type'),
            'timestamp': data.get('timestamp'),
        ...
        for src in data.get('sources', []):
            ...
            share_data['sources'].append(source_data)
        
        # Encode to base64
        json_str = json.dumps(share_data)
        encoded = base64.urlsafe_b64encode(json_str.encode()).decode()
        
        share_url = f"{request.host_url}shared/{encoded}"
...
def shared_results(encoded):
    try:
        ...
        logger.error(f"Shared results error: {e}")
        return render_template('error.html', error='Invalid or expired share link'), 404
...
def not_found(e):
    if request.path.startswith('/shared'):
        return render_template('error.html', error='404 - Not Found', description='The shared link is invalid or expired.'), 404
